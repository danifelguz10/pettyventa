...
import 'package:pattyventas/saleslist_screen.dart';
...
class _SalesScreenState extends State<SalesScreen> {
  ...
  Future<void> loadProducts() async {
    ...
    docs.forEach((doc) {
      ...
      String imageUrl = data['imageurl'] ?? 'URL de Imagen Desconocida';
  ...
  void registerSale() async {
    ...
    for (var saleProduct in selectedProducts) {
      ...
      Product productInInventory = products.firstWhere(
        (product) => product.productId == saleProduct.productId,
        orElse: () => Product(
      ...
      if (productInInventory.inventoryQuantity >= saleProduct.quantity) {
        productInInventory.inventoryQuantity -= saleProduct.quantity;
        updatedInventory.add({
          'productId': productInInventory.productId,
          'inventoryQuantity': productInInventory.inventoryQuantity,
        });
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
                'No hay suficientes unidades disponibles de ${productInInventory.name}'),
          ),
        );
        return;
      }
      print('Inventario actualizado para ${productInInventory.name}, '
          'Nueva cantidad: ${productInInventory.inventoryQuantity}');
    }
    ...
    try {
      ...
      // Actualizar cantidades en el inventario en Firestore
